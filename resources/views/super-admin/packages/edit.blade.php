...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-lg shadow-md">
        <div class="px-6 py-4 border-b border-gray-200">
            <h2 class="text-lg font-semibold text-gray-900">Edit: {{ $package->name }}</h2>
        </div>

        <form action="{{ route('super-admin.packages.update', $package) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">{{ old('description', $package->description) }}</textarea>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Monthly Price (₹) *</label>
                    <input type="number" name="monthly_price" value="{{ old('monthly_price', $package->monthly_price) }}" step="0.01" min="0" required
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Annual Price (₹) *</label>
                    <input type="number" name="annual_price" value="{{ old('annual_price', $package->yearly_price) }}" step="0.01" min="0" required
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Lifetime Price (₹)</label>
                    <input type="number" name="lifetime_price" value="{{ old('lifetime_price', $package->lifetime_price ?? '') }}" step="0.01" min="0"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Max Flats</label>
                    <input type="number" name="max_flats" value="{{ old('max_flats', $package->max_flats) }}" min="1"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500"
