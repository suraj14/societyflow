...
<div class="container mx-auto px-4 py-6">
    <!-- Header -->
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-900">My Villa</h1>
        <p class="text-gray-600 mt-1">View your villa details</p>
    </div>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-lg shadow-md overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-purple-50 to-blue-50">
                        <h2 class="text-lg font-semibold text-gray-900">Villa Details</h2>
                    </div>
                    <div class="px-6 py-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <!-- Villa Name -->
                            <div>
                                <label class="block text-sm font-medium text-gray-500 mb-1">Villa Name</label>
                                <p class="text-lg font-semibold text-gray-900">{{ $villa->name }}</p>
                            </div>

                            <!-- Villa Area -->
                            <div>
                                <label class="block text-sm font-medium text-gray-500 mb-1">Villa Area</label>
                                <p class="text-lg font-semibold text-gray-900">{{ $villa->villaArea->name ?? 'N/A' }}</p>
                            </div>

                            <!-- Society -->
                            <div>
                                <label class="block text-sm font-medium text-gray-500 mb-1">Society</label>
                                <p class="text-lg font-semibold text-gray-900">{{ $villa->society->name ?? 'N/A' }}</p>
                            </div>

                            <!-- Plot Number -->
                            <div>
                                <label class="block text-sm font-medium text-gray-500 mb-1">Plot Number</label>
                                <p class="text-lg font-semibold text-gray-900">{{ $villa->plot_number ?? 'N/A' }}</p>
                            </div>

                            <!-- Bedrooms -->
                            <div>
                                <label class="block text-sm font-medium text-gray-500 mb-1">Bedrooms</label>
                                <p class="text-lg font-semibold text-gray-900">{{ $villa->bedrooms ?? 'N/A' }}</p>
                            </div>

                            <!-- Bathrooms -->
                            <div>
                                <label class="block text-sm font-medium text-gray-500 mb-1">Bathrooms</label>
                                <p class="text-lg font-semibold text-gray-900">{{ $villa->bathrooms ?? 'N/A' }}</p>
                            </div>

                            <!-- Area (Sq Ft) -->
                            <div>
                                <label class="block text-sm font-medium text-gray-500 mb-1">Area (Sq Ft)</label>
                                <p class="text-lg font-semibold text-gray-900">{{ $villa->area ?? 'N/A' }}</p>
                            </div>
                            ...
                            <div>
                                ...
                                <div>
                                    ...
                                    @endphp
                                    <span class="px-3 py-1 text-sm font-semibold rounded-full {{ $statusColors[$villa->status] ?? 'bg-gray-100 text-gray-800' }}">
                                        {{ ucfirst($villa->status ?? 'N/A') }}
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        @if($villa->description)
                            <div class="mt-6 pt-6 border-t border-gray-200">
                                ...
                                <p class="text-gray-700">{{ $villa->description }}</p>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="bg-white rounded-lg shadow-md p-6">
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center justify-between pb-4 border-b border-gray-200">
                            ...
                            <span class="font-medium text-gray-900">{{ $villa->id }}</span>
                        </div>
                        <div class="flex items-center justify-between pb-4 border-b border-gray-200">
                            <span class="text-gray-600">Plot Number</span>
                            <span class="font-medium text-gray-900">{{ $villa->plot_number ?? 'N/A' }}</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-gray-600">Total Residents</span>
                            <span class="font-medium text-gray-900">{{ $villa->residents->count() ?? 0 }}</span>
                        </div>
                    </div>
                </div>
