    ...
    <div class="max-w-3xl mx-auto">
        ...
        <div class="bg-white rounded-lg shadow-md">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-lg font-semibold text-gray-800">Create New Owner</h2>
            </div>

            <form action="{{ route('owners.store') }}" method="POST" class="p-6" enctype="multipart/form-data">
                @csrf
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="notes" rows="3"
                                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                  placeholder="Additional notes about the owner">{{ old('notes') }}</textarea>
                        @error('notes')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function fetchFlats() {
        ...
        fetch(`/api/owners/flats?building_id=${buildingId}&floor=${floor}`)
            ...
            .then(data => {
                ...
                const currentValue = flatNoSelect.value;
                flatNoSelect.innerHTML = '<option value="">Select Flat No.</option>';
                if (data && data.length > 0) {
                    data.forEach(flatNo => {
                        const option = document.createElement('option');
    ...
    function fetchVillaAreas() {
        ...
        fetch(`/api/owners/villa-areas?society_id=${societyId}`)
            ...
            .then(data => {
                ...
                const currentValue = villaAreaSelect.value;
                villaAreaSelect.innerHTML = '<option value="">Select Villa Area</option>';
                if (data && data.length > 0) {
                    data.forEach(area => {
                        const option = document.createElement('option');
    ...
    function fetchVillaNumbers() {
        ...
        fetch(`/api/owners/villa-numbers?villa_area_id=${villaAreaId}`)
            ...
            .then(data => {
                ...
                const currentValue = villaNoSelect.value;
                villaNoSelect.innerHTML = '<option value="">Select Villa No.</option>';
                if (data && data.length > 0) {
    ...
    if (propertyTypeSelect.value === 'apartment') {
        ...
        // If no property type selected, hide both
        apartmentFields.style.display = 'none';
        villaFields.style.display = 'none';
    }

    // Debug: Log initial state
    console.log('Property Type:', propertyTypeSelect.value);
    console.log('Society ID:', getSocietyId());
