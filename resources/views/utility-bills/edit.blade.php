...
@section('page-title', 'Edit Utility Bill')
...
<div class="container mx-auto px-4 py-6">
    <div class="flex items-center mb-6">
        ...
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Edit Utility Bill</h1>
            <p class="text-gray-600 mt-1">{{ $utilityBill->flat->flat_number ?? 'N/A' }} - {{ $utilityBill->bill_type }}</p>
        </div>
    </div>

    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
            <form action="{{ route('utility-bills.update', $utilityBill) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select name="flat_id" id="flat_id" 
                        ...
                        @foreach($flats as $flat)
                            <option value="{{ $flat->id }}" {{ old('flat_id', $utilityBill->flat_id) == $flat->id ? 'selected' : '' }}>
                                {{ $flat->flat_number }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="bill_type" id="bill_type" 
                        ...
                        @foreach($billTypes as $type)
                            <option value="{{ $type }}" {{ old('bill_type', $utilityBill->bill_type) == $type ? 'selected' : '' }}>
                                {{ $type }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="date" name="bill_date" id="bill_date"
                           value="{{ old('bill_date', $utilityBill->bill_date->format('Y-m-d')) }}"
                ...
                <div>
                    ...
                    <div class="mb-3">
                        ...
                        <a href="{{ asset('storage/' . $utilityBill->bill_file_path) }}" 
                           target="_blank"
                           class="inline-flex items-center px-3 py-2 bg-blue-50 border border-blue-200 rounded-lg text-blue-600 hover:bg-blue-100 transition-colors">
                ...
                <div>
                    ...
                    <select name="status" id="status" 
                            ...
                            required>
                        <option value="unpaid" {{ old('status', $utilityBill->status) == 'unpaid' ? 'selected' : '' }}>Unpaid</option>
                        <option value="partial" {{ old('status', $utilityBill->status) == 'partial' ? 'selected' : '' }}>Partial</option>
                        <option value="paid" {{ old('status', $utilityBill->status) == 'paid' ? 'selected' : '' }}>Paid</option>
                    </select>
                    @error('status')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="3"
                              ...
                              placeholder="Add any additional notes (optional)">{{ old('notes', $utilityBill->notes) }}</textarea>
                    @error('notes')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
