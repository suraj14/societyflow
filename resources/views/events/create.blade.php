            ...
            <div>
                ...
                <div class="space-y-2">
                    <label class="flex items-center">
                        <input type="radio" name="is_role_based" value="1" {{ old('is_role_based', 1) == 1 ? 'checked' : '' }} 
                    ...
                    <label class="flex items-center">
                        <input type="radio" name="is_role_based" value="0" {{ old('is_role_based') == 0 ? 'checked' : '' }} 
                               class="rounded border-gray-300 text-purple-600 focus:ring-purple-500" onchange="updateVisibilityFields()">
                    ...
                    @foreach($roles as $role)
                        <label class="flex items-center">
                            <input type="checkbox" name="visible_roles[]" value="{{ $role->name }}" 
                                   {{ in_array($role->name, old('visible_roles', [])) ? 'checked' : '' }}
                    ...
                    @foreach($users as $user)
                        <label class="flex items-center">
                            <input type="checkbox" name="visible_users[]" value="{{ $user->id }}" 
                                   {{ in_array($user->id, old('visible_users', [])) ? 'checked' : '' }}
                                   class="rounded border-gray-300 text-purple-600 focus:ring-purple-500">
