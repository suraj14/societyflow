...
<div class="container mx-auto px-4 py-6">
    <div class="flex items-center mb-6">
        ...
        <div>
            ...
            <p class="text-gray-600 mt-1">Add a new service provider to your society</p>
        </div>
    </div>

    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
            <form action="{{ route('service-providers.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <select name="service_id" id="service_id" 
                        ...
                        @if(isset($services) && $services->count() > 0)
                            ...
                            @endforeach
                        @else
                            <option value="" disabled>No services available - Please contact administrator</option>
                        @endif
                    </select>
                    @if(!isset($services) || $services->count() == 0)
                        <p class="text-orange-500 text-sm mt-1">
                            <i class="fas fa-exclamation-triangle mr-1"></i>
                            No services found. Please run: <code>php artisan db:seed --class=ServicesSeeder</code>
                        </p>
                    @endif
                    @error('service_id')
